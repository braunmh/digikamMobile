...
class _StatisticKeywordState extends State<StatisticKeywordDialog> {
                          ...
                          children: <Widget>[
                            ...
                            TextFormField(
                              ...
                              decoration: InputDecoration(
                                  labelText: AppLocalizations.of(context)!.statisticKeywordYear
                              ),
                                ...
                                if (value != null && value.isNotEmpty) {
                                  try {
                                    ...
                                    if (year > DateTime.now().year) {
                                      year = 0;
                                      return AppLocalizations.of(context)!.statisticKeywordValidationNoFuture;
                                    }
                                  } catch (e) {
                                    return AppLocalizations.of(context)!.statisticKeywordValidationYear;
                                  }
                                }
                                return null;
                              },
                                ...
                                ElevatedButton(
                                  onPressed: () {
                                    if (isValid()) {
                                      ...
                                      SnackBar snackBar = SnackBar(
                                          content: Text(AppLocalizations.of(context)!.statisticKeywordValidationAll));
                                      ScaffoldMessenger.of(context)
                                          .showSnackBar(snackBar);
                                    }
                                  },
                                  child: Text(AppLocalizations.of(context)!.statisticKeywordGenerate),
                                ),
                                ElevatedButton(
                                  onPressed: () {
  ...
  Widget displayResult(BuildContext context, List<StatisticKeyword> result) {
    if (result.isEmpty) {
      ...
      return DataTable(columns: <DataColumn>[
        DataColumn(label: Text(AppLocalizations.of(context)!.statisticKeywordKeyword)),
        DataColumn(label: Text(AppLocalizations.of(context)!.statisticKeywordCount)),
      ], rows: getRows(result));
    }
  }
  ...
  Widget buildKeywordDropDown(BuildContext context) {
    return TypeAheadField<Keyword>(
        ...
        itemBuilder: (context, Keyword item) => Text(item.name),
  ...
  Future<List<Keyword>> getKeywordSuggestions(String pattern) async {
    ...
    return keywords.where((k) => k.name.toLowerCase().contains(pattern)).toList();
  }
}
